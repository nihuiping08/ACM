...
连接a,b字符串，中间使用一个多余的字符串，在结尾处添加多一个字符串，使得比较时不用判断是否越界
因此，求完后缀数组sa之后，最长公共子串必定是最大的height数组的值，另外那个height的两个suffix(i)
和suffix(i-1)必定不在同一个串才行。这里注意到中间连接字符，那么sa必定相差前一个串的长度个。
