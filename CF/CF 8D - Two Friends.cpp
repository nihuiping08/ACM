...
题目：
    平面上三个点A,B,C，起初Alice和Bob都在A点，现在Alice需要经过B点然后
    到达C点，而Bob直接到达C点。现在设计路线，使得两人的最长公共部分最长
    （一旦分开，以后就算再走到一起也不算）

分析：
    我们设答案为s，所以我们知道Alice和Bob分离点pos在以A为圆心，s为半径
    的圆上。我们发现pos明显具有单调性。
    我们可以首先二分位置，在BC的线段上的点D，然后再二分出pos在线段AD的
    具体位置，然后判断该位置是否符合要求就行了。
